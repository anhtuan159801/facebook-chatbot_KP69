...
class RAGAnythingConfig:
    ...
    # API Configuration (for 'api' backend)
    # ---
    api_key: str = field(default=get_env_value("API_KEY", "", str))
    """API key for API-based models."""

    embedding_api_url: str = field(
        default=get_env_value("EMBEDDING_API_URL", "https://api.openai.com/v1/embeddings", str)
    )
    """Endpoint URL for the embedding API."""

    chat_api_url: str = field(
        default=get_env_value("CHAT_API_URL", "https://api.openai.com/v1/chat/completions", str)
    )
    """Endpoint URL for the chat completion API."""

    vision_api_url: str = field(
        default=get_env_value("VISION_API_URL", "https://api.openai.com/v1/chat/completions", str)
    )
    """Endpoint URL for the vision completion API."""
    
    embedding_model_name: str = field(
        default=get_env_value("EMBEDDING_MODEL_NAME", "text-embedding-3-small", str)
    )
    """Name of the embedding model for API calls."""

    chat_model_name: str = field(
        default=get_env_value("CHAT_MODEL_NAME", "gpt-4o", str)
    )
    """Name of the chat model for API calls."""

    vision_model_name: str = field(
        default=get_env_value("VISION_MODEL_NAME", "gpt-4o", str)
    )
    """Name of the vision model for API calls."""

    embedding_dimension: int = field(
        default=get_env_value("EMBEDDING_DIMENSION", 1536, int)
    )
    """Dimension of the vector embeddings (e.g., 1536 for text-embedding-3-small)."""
